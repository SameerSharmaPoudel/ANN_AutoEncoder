...
def compute_error_metrics(target, prediction):
    ...
    ae, se, rae = 0.0, 0.0, 0.0
    
    for i in range(len(prediction)):  #loops through the length of batchsize
      
        ae += np.abs(prediction[i] - target[i])
        
        se += (prediction[i] - target[i])**2
        
        rae += np.abs(prediction[i] - target[i]) / target[i]
                      
    mae = np.mean(ae / len(prediction))
    mse = np.mean(se / len(prediction))
    mape = np.mean(rae / len(prediction)) * 100
